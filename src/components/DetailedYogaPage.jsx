...
const DetailedYogaPage = () => {
  const location = useLocation();
  const navigate = useNavigate();

  const { title, gifSrc, description, advantages, howToPerform } =
    location.state;
  ...
  const handleStartPose = () => {
    navigate("/yoga-camera", {
      state: { title, gifSrc, description },
    });
  };
  return (
    <Box bg={bgColor} minHeight="100vh" py={8}>
      <Container maxW="6xl">
        ...
        <Button mt={4} size="lg" colorScheme="blue" onClick={handleStartPose}>
          Start Pose
        </Button>
      </Container>
    </Box>
...
export default DetailedYogaPage;
